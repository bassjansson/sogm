...
float ** fourierTransform(float * sig, int window) {	
	...
	for(int b = 0; b < specwindow; b++) {				// walk through the bins (spectrum)
		...
		float alpha = (b + 1) * 2*M_PI / float(window);	// binnumber * 2pi / window
		...
		for(int i = 0; i < window; i++) {
			amps[0] += cos(alpha * i) * sig[i];
			amps[1] += sin(alpha * i) * sig[i];
		}
...
float * inverseFourierTransform(float ** pol, int specwindow) {	
	int window = specwindow*2;
